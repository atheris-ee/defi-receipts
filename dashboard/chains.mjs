// EVM chain registry — RPC endpoints (paid DRPC, key from env) and a curated token list per chain.
// Only tokens the defi-tracker scanner mentions plus common stables/LSTs.
// Native balances are queried separately via eth_getBalance.

const DRPC_KEY = process.env.DRPC_KEY || '';
const drpc = (network) => `https://lb.drpc.org/ogrpc?network=${network}&dkey=${DRPC_KEY}`;

export const CHAINS = {
  ethereum: {
    label: 'Ethereum',
    rpc: drpc('ethereum'),
    nativeSymbol: 'ETH',
    nativeCoinId: 'coingecko:ethereum',
    tokens: {
      USDC:   { addr: '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', decimals: 6 },
      USDT:   { addr: '0xdAC17F958D2ee523a2206206994597C13D831ec7', decimals: 6 },
      DAI:    { addr: '0x6B175474E89094C44Da98b954EedeAC495271d0F', decimals: 18 },
      WETH:   { addr: '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2', decimals: 18 },
      WBTC:   { addr: '0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599', decimals: 8 },
      WSTETH: { addr: '0x7f39C581F595B53c5cb19bD0b3f8dA6c935E2Ca0', decimals: 18 },
      STETH:  { addr: '0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84', decimals: 18 },
      USDS:   { addr: '0xdC035D45d973E3EC169d2276DDab16f1e407384F', decimals: 18 },
    },
  },
  polygon: {
    label: 'Polygon',
    rpc: drpc('polygon'),
    nativeSymbol: 'POL',
    nativeCoinId: 'coingecko:matic-network',
    tokens: {
      'USDC.e': { addr: '0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174', decimals: 6 },
      USDC:     { addr: '0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359', decimals: 6 },
      USDT:     { addr: '0xc2132D05D31c914a87C6611C10748AEb04B58e8F', decimals: 6 },
      DAI:      { addr: '0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063', decimals: 18 },
      WETH:     { addr: '0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619', decimals: 18 },
      WMATIC:   { addr: '0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270', decimals: 18 },
      WBTC:     { addr: '0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6', decimals: 8 },
    },
  },
  arbitrum: {
    label: 'Arbitrum',
    rpc: drpc('arbitrum'),
    nativeSymbol: 'ETH',
    nativeCoinId: 'coingecko:ethereum',
    tokens: {
      USDC:     { addr: '0xaf88d065e77c8cC2239327C5EDb3A432268e5831', decimals: 6 },
      'USDC.e': { addr: '0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8', decimals: 6 },
      USDT:     { addr: '0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9', decimals: 6 },
      ARB:      { addr: '0x912CE59144191C1204E64559FE8253a0e49E6548', decimals: 18 },
      WETH:     { addr: '0x82aF49447D8a07e3bd95BD0d56f35241523fBab1', decimals: 18 },
      WBTC:     { addr: '0x2f2a2543B76A4166549F7aaB2e75Bef0aefC5B0f', decimals: 8 },
      WSTETH:   { addr: '0x5979D7b546E38E414F7E9822514be443A4800529', decimals: 18 },
    },
  },
  base: {
    label: 'Base',
    rpc: drpc('base'),
    nativeSymbol: 'ETH',
    nativeCoinId: 'coingecko:ethereum',
    tokens: {
      USDC:  { addr: '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913', decimals: 6 },
      DAI:   { addr: '0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb', decimals: 18 },
      WETH:  { addr: '0x4200000000000000000000000000000000000006', decimals: 18 },
      CBETH: { addr: '0x2Ae3F1Ec7F1F5012CFEab0185bfc7aa3cf0DEc22', decimals: 18 },
      CBBTC: { addr: '0xcbB7C0000aB88B473b1f5aFd9ef808440eed33Bf', decimals: 8 },
    },
  },
  optimism: {
    label: 'Optimism',
    rpc: drpc('optimism'),
    nativeSymbol: 'ETH',
    nativeCoinId: 'coingecko:ethereum',
    tokens: {
      USDC:     { addr: '0x0b2C639c533813f4Aa9D7837CAf62653d097Ff85', decimals: 6 },
      'USDC.e': { addr: '0x7F5c764cBc14f9669B88837ca1490cCa17c31607', decimals: 6 },
      USDT:     { addr: '0x94b008aA00579c1307B0EF2c499aD98a8ce58e58', decimals: 6 },
      OP:       { addr: '0x4200000000000000000000000000000000000042', decimals: 18 },
      WETH:     { addr: '0x4200000000000000000000000000000000000006', decimals: 18 },
      DAI:      { addr: '0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1', decimals: 18 },
    },
  },
  avalanche: {
    label: 'Avalanche',
    rpc: drpc('avalanche'),
    nativeSymbol: 'AVAX',
    nativeCoinId: 'coingecko:avalanche-2',
    tokens: {
      USDC:    { addr: '0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E', decimals: 6 },
      USDT:    { addr: '0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7', decimals: 6 },
      WAVAX:   { addr: '0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7', decimals: 18 },
      WETH:    { addr: '0x49D5c2BdFfac6CE2BFdB6640F4F80f226bc10bAB', decimals: 18 },
      'BTC.B': { addr: '0x152b9d0FdC40C096757F570A51E494bd4b943E50', decimals: 8 },
    },
  },
  bsc: {
    label: 'BSC',
    rpc: drpc('bsc'),
    nativeSymbol: 'BNB',
    nativeCoinId: 'coingecko:binancecoin',
    // Note: BSC stablecoins are 18-decimal (not 6 like other chains).
    tokens: {
      USDC: { addr: '0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d', decimals: 18 },
      USDT: { addr: '0x55d398326f99059fF775485246999027B3197955', decimals: 18 },
      WBNB: { addr: '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c', decimals: 18 },
      ETH:  { addr: '0x2170Ed0880ac9A755fd29B2688956BD959F933F8', decimals: 18 },
      BTCB: { addr: '0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c', decimals: 18 },
    },
  },
};

// Scanner uses these chain names (extracted from action strings). Map them to keys above.
export const CHAIN_NAME_TO_KEY = {
  ethereum: 'ethereum',
  polygon: 'polygon',
  arbitrum: 'arbitrum',
  base: 'base',
  'op mainnet': 'optimism',
  optimism: 'optimism',
  avalanche: 'avalanche',
  bsc: 'bsc',
  binance: 'bsc',
};
